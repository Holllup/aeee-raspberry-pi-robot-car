...
function bindBtn(id, cmd, left, right) {
    ...
    const start = (e) => { 
        ...
        if(!isActive) {
            ...
            statusEl.innerText = `发送指令: [${cmd}] L:${left} R:${right}`;
            sendCmd(cmd, left, right); 
        }
    };
...
// 绑定四个方向
bindBtn('btn-w', 'drive', base_speed, base_speed);
bindBtn('btn-s', 'drive', -base_speed, -base_speed);
bindBtn('btn-a', 'drive', -turn_speed, turn_speed);
bindBtn('btn-d', 'drive', turn_speed, -turn_speed);
