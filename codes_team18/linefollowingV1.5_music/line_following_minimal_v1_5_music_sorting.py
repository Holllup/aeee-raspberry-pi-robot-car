...
SORT_BLUE_SAT_LOW = 20
SORT_BLUE_VAL_LOW = 20
SORT_BLUE_RGB_CENTER = np.array([104, 114, 153], dtype=np.int16)
SORT_BLUE_RGB_DISTANCE_THRESHOLD = 120
...
SORT_MIN_CONTOUR_AREA = 180
SORT_MIN_BOX_SIZE = 15
...
def contour_is_square_by_corners(contour):
    ...
    if h <= 0:
        return False
    aspect_ratio = w / float(h)
    return SORT_SQUARE_MIN_ASPECT <= aspect_ratio <= SORT_SQUARE_MAX_ASPECT
...
def count_sorting_objects(warped_sign_bgr):
    ...
    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
    blue_mask_hsv = cv2.inRange(
        hsv,
        np.array([SORT_BLUE_HUE_LOW, SORT_BLUE_SAT_LOW, SORT_BLUE_VAL_LOW], dtype=np.uint8),
        np.array([SORT_BLUE_HUE_HIGH, 255, 255], dtype=np.uint8),
    )
    blue_mask = refine_mask_by_rgb_distance(
        roi, blue_mask_hsv, SORT_BLUE_RGB_CENTER, SORT_BLUE_RGB_DISTANCE_THRESHOLD
    )
...
def main():
    ...
    try:
        ...
        while True:
            ...
            sign_result = None
            traffic_light_result = None
            allow_purple_trigger = now >= traffic_release_ignore_until
