...
SIGN_TEMPLATE_MATCH_THRESHOLD = 0.70
...
def load_alarm_template():
    template_candidates = (
        Path(__file__).with_name(ALARM_TEMPLATE_FILENAME),
        Path(__file__).resolve().parent.parent / "templates" / ALARM_TEMPLATE_FILENAME,
    )
    template_path = next(
...
def classify_sign_symbol(frame_bgr, sign_roi):
    ...
    template = load_alarm_template()
    if template is None:
        return {
            "label": "UNKNOWN",
            "confidence": 0.0,
            "reason": "missing_alarm_template",
            "binary": binary,
        }

    if binary.shape != template.shape:
        binary = cv2.resize(binary, (template.shape[1], template.shape[0]), interpolation=cv2.INTER_NEAREST)

    match_score = float(cv2.matchTemplate(binary, template, cv2.TM_CCOEFF_NORMED)[0, 0])
    if match_score >= SIGN_TEMPLATE_MATCH_THRESHOLD:
        return {
            "label": "ALARM",
    ...
    return {
        "label": "UNKNOWN",
        "confidence": max(0.0, match_score),
        "reason": "template_match_below_threshold",
        "binary": binary,
        "match_score": match_score,
    }
    ...
    try:
        ...
        while True:
            ...
            draw_overlay(
                ...
                purple_detection,
                center_purple_detection,
                center_purple_sign_roi if center_purple_sequence_active else sign_roi,
