...
def detect_traffic_light_state(frame_bgr, sign_roi):
    ...
    bright_threshold = int(max(185, min(245, max_val - 30)))
    ...
    contours = [c for c in contours if cv2.contourArea(c) >= TRAFFIC_LIGHT_COLOR_MIN_AREA]
    if not contours:
    ...
    if moments["m00"] > 0:
        ...
        cx = x + w // 2
        cy = y + h // 2
    cx = int(clamp(cx, 0, roi.shape[1] - 1))
    cy = int(clamp(cy, 0, roi.shape[0] - 1))

    # The centroid may land on the lamp's white core. Use saturated bright pixels
    # inside the bright contour to estimate lamp color more reliably.
    ...
    if np.any(contour_pixels):
        bright_v = v_channel[contour_pixels]
        v_cut = int(np.percentile(bright_v, 70))
        candidate = contour_pixels & (v_channel >= v_cut) & (s_channel >= 35)
        if not np.any(candidate):
            candidate = contour_pixels & (s_channel >= 25)
        if np.any(candidate):
            ys, xs = np.where(candidate)
            sat = s_channel[ys, xs].astype(np.int32)
            val = v_channel[ys, xs].astype(np.int32)
            best_idx = int(np.argmax(sat * 3 + val))
            px = int(xs[best_idx])
            py = int(ys[best_idx])
            patch = roi[max(0, py - 1) : min(roi.shape[0], py + 2), max(0, px - 1) : min(roi.shape[1], px + 2)]
            mean_bgr = patch.reshape(-1, 3).mean(axis=0)
            b, g, r = [int(round(v)) for v in mean_bgr]
        else:
            b, g, r = [int(v) for v in roi[cy, cx]]
    else:
        b, g, r = [int(v) for v in roi[cy, cx]]
    ...
    if r >= g + 10 and r >= b + 10:
        ...
        state = "GREEN"
    elif abs(r - g) <= 12 and min(r, g) >= b + 15:
        # Ambiguous warm white/yellow core: fallback to vertical position.
        state = "RED" if cy < (roi.shape[0] * 0.5) else "GREEN"

    abs_rect = (roi_x0 + x, roi_y0 + y, w, h)
    center = (roi_x0 + cx, roi_y0 + cy)
    return {"state": state, "rect": abs_rect, "center": center, "center_bgr": (b, g, r)}
    ...
    try:
        ...
        while True:
            ...
            if traffic_wait_active:
                if traffic_wait_sign_roi is not None:
                ...
                if (
                    ...
                    and traffic_light_result["state"] == "GREEN"
