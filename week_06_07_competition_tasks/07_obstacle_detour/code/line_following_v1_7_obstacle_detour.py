...
OBSTACLE_DETOUR_SECONDS = 3.0
OBSTACLE_RELEASE_IGNORE_SECONDS = 2.0
OBSTACLE_DETOUR_TURN_SPEED = 52
OBSTACLE_DETOUR_FORWARD_SPEED = 32
# Obstacle detour sequence requested:
# 1) right turn ~60 deg, 2) forward 3s, 3) left turn ~120 deg, 4) forward until line is reacquired.
OBSTACLE_RIGHT_TURN_SECONDS = 2.0
OBSTACLE_FORWARD_1_SECONDS = 4.0
OBSTACLE_LEFT_TURN_SECONDS = 3.0
OBSTACLE_SEEK_LINE_SPEED = 30
OBSTACLE_LINE_REACQUIRE_HITS = 4
OBSTACLE_DETOUR_PHASES = (
    ("right_5s", OBSTACLE_RIGHT_TURN_SECONDS),
    ("forward_5s", OBSTACLE_FORWARD_1_SECONDS),
    ("left_7s", OBSTACLE_LEFT_TURN_SECONDS),
    ("seek_line", 999.0),
)
...
SIGN_SYMBOL_SAT_THRESHOLD = 35
SIGN_SYMBOL_VAL_MAX = 245
...
OBSTACLE_TEMPLATE_MATCH_THRESHOLD = 0.60
OBSTACLE_TEMPLATE_MARGIN = 0.02
...
OBSTACLE_TEMPLATE_FILENAME = "obstacle_template.png"
...
class ControlState:
    ...
    def set_manual_drive(self, left_speed, right_speed, until_time, mode="manual"):
        with self.lock:
            self.manual_left_speed = int(clamp(left_speed, -MAX_SPEED, MAX_SPEED))
            self.manual_right_speed = int(clamp(right_speed, -MAX_SPEED, MAX_SPEED))
            self.manual_until = until_time
            self.manual_mode = mode

    def clear_manual_drive(self):
        with self.lock:
            self.manual_left_speed = 0
            self.manual_right_speed = 0
            self.manual_until = -999.0
...
def preprocess_sign_symbol(warped_sign_bgr):
    ...
    pink_hue_mask = ((hsv[:, :, 0] >= 145) | (hsv[:, :, 0] <= 12))
    dark_mask = gray <= SIGN_SYMBOL_VAL_MAX
    # Under glare, saturation can drop; include pink-ish hue fallback.
...
def classify_sign_symbol(frame_bgr, sign_roi):
    ...
    templates = {
        ...
        "OBSTACLE": load_sign_template(OBSTACLE_TEMPLATE_FILENAME),
    }
    ...
    obstacle_score = scores.get("OBSTACLE", -1.0)
    fallback_score = classify_obstacle_fallback(binary)
    other_scores = [score for label, score in scores.items() if label != "OBSTACLE"]
    other_best = max(other_scores) if other_scores else -1.0
    best_non_traffic = max(
        [score for label, score in scores.items() if label != "TRAFFIC"],
        default=-1.0,
    )
    ...
    if (
        ...
        and (
            obstacle_score < OBSTACLE_TEMPLATE_MATCH_THRESHOLD
            or fallback_score >= best_non_traffic + 0.05
        )
    ):
        return {
            "label": "OBSTACLE",
            "confidence": max(fallback_score, obstacle_score),
            "reason": "fallback_shape_match_obstacle",
            "binary": binary,
            "match_score": max(fallback_score, obstacle_score),
            "scores": scores,
        }

    if (
        obstacle_score >= OBSTACLE_TEMPLATE_MATCH_THRESHOLD
        and obstacle_score >= other_best + OBSTACLE_TEMPLATE_MARGIN
    ):
        return {
            "label": "OBSTACLE",
            "confidence": obstacle_score,
            "reason": "xor_template_match_obstacle",
            "binary": binary,
            "match_score": obstacle_score,
            "scores": scores,
        }
    ...
    if fallback_score is not None and best_score < SIGN_TEMPLATE_MATCH_THRESHOLD:
        return {
            "label": "OBSTACLE",
...
def classify_obstacle_fallback(binary):
    ...
    contours = [c for c in contours if cv2.contourArea(c) >= frame_area * 0.004]
    if not contours:
        return None
    ...
    if main_area < frame_area * 0.07 or main_area > frame_area * 0.80:
        return None
    ...
    if bw <= 0 or bh <= 0:
        return None
    aspect = bh / float(bw)
    if aspect < 0.65 or aspect > 3.2:
        return None

    hull = cv2.convexHull(main)
    hull_area = float(cv2.contourArea(hull))
    solidity = (main_area / hull_area) if hull_area > 1 else 0.0
    extent = main_area / float(max(1, bw * bh))
    if not (0.24 <= extent <= 0.94 and 0.22 <= solidity <= 0.995):
        return None

    has_head_like = False
    if len(contours) >= 2:
        c2 = contours[1]
        area2 = float(cv2.contourArea(c2))
        if area2 >= frame_area * 0.006:
            x2, y2, w2, h2 = cv2.boundingRect(c2)
            c2_aspect = h2 / float(max(1, w2))
            c2_cx = x2 + w2 / 2.0
            c2_cy = y2 + h2 / 2.0
            main_cx = x + bw / 2.0
            main_cy = y + bh / 2.0
            if (
                0.65 <= c2_aspect <= 1.35
                and c2_cy < (main_cy - bh * 0.12)
                and abs(c2_cx - main_cx) <= bw * 0.45
            ):
                has_head_like = True
    ...
    if has_head_like:
        score += 0.24
    score += max(0.0, min(0.20, (aspect - 0.65) * 0.08))
    score += max(0.0, min(0.14, (0.90 - abs(extent - 0.56))))
...
def motor_control_loop(controller, control_state):
    ...
    while True:
        ...
        if state["pause_active"]:
            if state["motor_armed"] and loop_started <= state.get("manual_until", -999.0):
                controller.set_tank_drive(state["manual_left_speed"], state["manual_right_speed"])
            else:
                controller.hold_stop()
        elif state["motor_armed"] and loop_started <= state.get("manual_until", -999.0):
            controller.set_tank_drive(state["manual_left_speed"], state["manual_right_speed"])
...
def main():
    ...
    obstacle_detour_until = -999.0
    obstacle_detour_phase_index = -1
    obstacle_detour_phase_until = -999.0
    obstacle_detour_phase_name = ""
    obstacle_line_reacquire_hits = 0
    alarm_resume_until = -999.0
    fps = 0.0
    last_frame_time = time.perf_counter()
    last_obstacle_detect_time = -999.0
        ...
        while True:
            ...
            if center_purple_sequence_active and now >= center_purple_sequence_until:
                ...
                if sign_result is not None and sign_result.get("label") == "ALARM":
                    ...
                    music_command_status_until = -999.0
                    control_state.set_pause(True)
                elif sign_result is not None and sign_result.get("label") == "OBSTACLE":
                    activate_obstacle_detour(now)
                    last_obstacle_detect_time = now

            if (
                not center_purple_sequence_active
                and not traffic_wait_active
                and not obstacle_detour_active
                and now >= sorting_post_cooldown_until
                and (now - last_obstacle_detect_time) >= 0.45
            ):
                scan_sign_roi = detect_sign_roi(frame)
                if scan_sign_roi is not None:
                    scan_sign_result = classify_sign_symbol(frame, scan_sign_roi)
                    if scan_sign_result.get("label") == "OBSTACLE":
            ...
            if obstacle_detour_active:
                if obstacle_detour_phase_name == "seek_line":
                    ...
                    control_state.set_manual_drive(
                        OBSTACLE_SEEK_LINE_SPEED,
                        OBSTACLE_SEEK_LINE_SPEED,
                        now + 0.20,
                        mode="obstacle_seek_line",
                    )
                    if obstacle_line_reacquire_hits >= OBSTACLE_LINE_REACQUIRE_HITS:
                        ...
                        traffic_release_ignore_until = max(
                            ...
                            now + OBSTACLE_RELEASE_IGNORE_SECONDS,
                        )
                        control_state.set_pause(False)

                if now >= obstacle_detour_phase_until:
                    obstacle_detour_phase_index += 1
                    if obstacle_detour_phase_index >= len(OBSTACLE_DETOUR_PHASES):
                        obstacle_detour_active = False
                        obstacle_detour_until = -999.0
                        obstacle_detour_phase_until = -999.0
                        obstacle_detour_phase_name = "done"
                        ...
                        phase_name, phase_duration = OBSTACLE_DETOUR_PHASES[obstacle_detour_phase_index]
                        obstacle_detour_phase_name = phase_name
                        obstacle_detour_phase_until = now + phase_duration
                        if phase_name == "right_5s":
                            left_cmd = OBSTACLE_DETOUR_TURN_SPEED
                            right_cmd = -OBSTACLE_DETOUR_TURN_SPEED
                        elif phase_name == "left_7s":
                            left_cmd = -OBSTACLE_DETOUR_TURN_SPEED
                            right_cmd = OBSTACLE_DETOUR_TURN_SPEED
                        elif phase_name == "seek_line":
                            left_cmd = OBSTACLE_SEEK_LINE_SPEED
                            right_cmd = OBSTACLE_SEEK_LINE_SPEED
                            obstacle_detour_phase_until = obstacle_detour_until
                        else:
                            left_cmd = OBSTACLE_DETOUR_FORWARD_SPEED
                            right_cmd = OBSTACLE_DETOUR_FORWARD_SPEED
                        control_state.set_manual_drive(
                            left_cmd,
                            right_cmd,
                            obstacle_detour_phase_until,
                            mode=f"obstacle_{phase_name}",
                        )
                elif now >= obstacle_detour_until:
                    obstacle_detour_active = False
                    obstacle_detour_until = -999.0
                    obstacle_detour_phase_until = -999.0
                    obstacle_detour_phase_name = "timeout"
                    control_state.clear_manual_drive()
                    traffic_release_ignore_until = max(
                        traffic_release_ignore_until,
                        now + OBSTACLE_RELEASE_IGNORE_SECONDS,
