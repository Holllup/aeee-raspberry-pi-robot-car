...
TRAFFIC_LIGHT_ROI_GAP_RATIO = 0.03
TRAFFIC_LIGHT_ROI_WIDTH_RATIO = 0.35
TRAFFIC_LIGHT_ROI_TOP_RATIO = 0.08
TRAFFIC_LIGHT_ROI_BOTTOM_RATIO = 0.92
TRAFFIC_LIGHT_COLOR_MIN_AREA = 40
TRAFFIC_LIGHT_HOUSING_MIN_AREA = 180
TRAFFIC_LIGHT_HOUSING_MIN_ASPECT_RATIO = 1.2
TRAFFIC_LIGHT_HOUSING_MAX_CENTER_Y_RATIO = 0.72
...
TRAFFIC_LIGHT_COLOR_VAL_LOW = 120
TRAFFIC_LIGHT_GREEN_LIT_VAL_LOW = 185
TRAFFIC_LIGHT_RED_LIT_VAL_LOW = 170
TRAFFIC_LIGHT_LIT_MEAN_VAL_LOW = 160
TRAFFIC_LIGHT_LIT_PIXEL_THRESHOLD = 120
...
def compute_drive_command(
    ...
    if entry_kind == "sharp" and entry_score >= ENTRY_ASSIST_SCORE_THRESHOLD:
        speed_scale = effective_base_speed / float(BASE_SPEED)
        base_speed = SHARP_ENTRY_BASE_SPEED * speed_scale
        steering_gain = SHARP_ENTRY_STEERING_GAIN
    elif entry_kind == "gentle" and entry_score >= ENTRY_ASSIST_SCORE_THRESHOLD:
        speed_scale = effective_base_speed / float(BASE_SPEED)
        base_speed = GENTLE_ENTRY_BASE_SPEED * speed_scale
...
def detect_traffic_light_state(frame_bgr, sign_roi):
    ...
    if sign_roi is not None:
        ...
        roi_x0 = clamp(x + w + int(w * TRAFFIC_LIGHT_ROI_GAP_RATIO), 0, frame_width - 1)
        roi_x1 = clamp(x + w + int(w * TRAFFIC_LIGHT_ROI_WIDTH_RATIO), roi_x0 + 1, frame_width)
        roi_y0 = clamp(y + int(h * TRAFFIC_LIGHT_ROI_TOP_RATIO), 0, frame_height - 1)
        roi_y1 = clamp(y + int(h * TRAFFIC_LIGHT_ROI_BOTTOM_RATIO), roi_y0 + 1, frame_height)
    else:
        roi_x0 = int(frame_width * 0.68)
        roi_x1 = frame_width
        roi_y0 = int(frame_height * 0.10)
        roi_y1 = int(frame_height * 0.95)
    ...
    dark_mask = cv2.inRange(
        ...
        np.array([179, 255, 90], dtype=np.uint8),
    )
    ...
    for contour in dark_contours:
        area = cv2.contourArea(contour)
        if area < TRAFFIC_LIGHT_HOUSING_MIN_AREA:
            continue
        x, y, w, h = cv2.boundingRect(contour)
        aspect_ratio = h / float(max(w, 1))
        center_y_ratio = (y + h * 0.5) / float(max(roi_height, 1))
        if aspect_ratio < TRAFFIC_LIGHT_HOUSING_MIN_ASPECT_RATIO:
            continue
        if center_y_ratio > TRAFFIC_LIGHT_HOUSING_MAX_CENTER_Y_RATIO:
            continue
        score = area * min(aspect_ratio, 3.0)
        if score > housing_score:
            housing_score = score
            housing_rect = (x, y, w, h)

    if housing_rect is None:
        return {"state": "NONE", "rect": (roi_x0, roi_y0, roi_x1 - roi_x0, roi_y1 - roi_y0), "center": None}

    hx, hy, hw, hh = housing_rect
    housing = roi[hy : hy + hh, hx : hx + hw]
    if housing.size == 0:
        return {"state": "NONE", "rect": (roi_x0 + hx, roi_y0 + hy, hw, hh), "center": None}

    housing_hsv = cv2.cvtColor(housing, cv2.COLOR_BGR2HSV)
    red_mask_1 = cv2.inRange(
        housing_hsv,
        np.array([TRAFFIC_LIGHT_RED_HUE_LOW_1, TRAFFIC_LIGHT_COLOR_SAT_LOW, TRAFFIC_LIGHT_RED_LIT_VAL_LOW], dtype=np.uint8),
        np.array([TRAFFIC_LIGHT_RED_HUE_HIGH_1, 255, 255], dtype=np.uint8),
    )
    red_mask_2 = cv2.inRange(
        housing_hsv,
        np.array([TRAFFIC_LIGHT_RED_HUE_LOW_2, TRAFFIC_LIGHT_COLOR_SAT_LOW, TRAFFIC_LIGHT_RED_LIT_VAL_LOW], dtype=np.uint8),
        np.array([TRAFFIC_LIGHT_RED_HUE_HIGH_2, 255, 255], dtype=np.uint8),
    )
    green_mask = cv2.inRange(
        housing_hsv,
        np.array([TRAFFIC_LIGHT_GREEN_HUE_LOW, TRAFFIC_LIGHT_COLOR_SAT_LOW, TRAFFIC_LIGHT_GREEN_LIT_VAL_LOW], dtype=np.uint8),
        np.array([TRAFFIC_LIGHT_GREEN_HUE_HIGH, 255, 255], dtype=np.uint8),
    )
    ...
    red_mean_val = float(cv2.mean(housing_hsv[:, :, 2], mask=red_mask)[0]) if red_pixels > 0 else 0.0
    green_mean_val = float(cv2.mean(housing_hsv[:, :, 2], mask=green_mask)[0]) if green_pixels > 0 else 0.0
    abs_rect = (roi_x0 + hx, roi_y0 + hy, hw, hh)
    center = (roi_x0 + hx + hw // 2, roi_y0 + hy + hh // 2)

    if (
        green_pixels >= TRAFFIC_LIGHT_LIT_PIXEL_THRESHOLD
        and green_mean_val >= TRAFFIC_LIGHT_LIT_MEAN_VAL_LOW
        and green_pixels > red_pixels * 1.2
    ):
        return {"state": "GREEN", "rect": abs_rect, "center": center}

    if (
        red_pixels >= TRAFFIC_LIGHT_LIT_PIXEL_THRESHOLD
        and red_mean_val >= TRAFFIC_LIGHT_LIT_MEAN_VAL_LOW - 10.0
        and red_pixels > green_pixels * 1.2
    ):
        return {"state": "RED", "rect": abs_rect, "center": center}

    return {"state": "NONE", "rect": abs_rect, "center": center}
...
def main():
    ...
    try:
        ...
        while True:
            ...
            right_speed = 0
            error_pixels = 0
            pause_active = center_purple_sequence_active or traffic_wait_active
