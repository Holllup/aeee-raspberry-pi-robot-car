...
class MazeNavigator:
    ...
    def _gate_forward_command(self, visual, mode_name, speed=None):
        ...
        steer = clamp(
            visual.gate_offset * self.config.marker_steer_gain * 12.0,
            -self.config.max_forward_steer,
            self.config.max_forward_steer,
        )
        return self._emit(speed + steer, speed - steer, mode_name)
    ...
    def _gate_forward_from_offset(self, offset, mode_name, speed=None):
        speed = self.config.follow_speed if speed is None else speed
        steer = clamp(
            offset * self.config.marker_steer_gain * 10.0,
            -self.config.max_forward_steer,
            self.config.max_forward_steer,
        )
        return self._emit(speed + steer, speed - steer, mode_name)
    ...
    def _gate_blind_drive_command(self, offset, mode_name):
        ...
        steer = clamp(
            offset * self.config.marker_steer_gain * 8.0 + 1.2,
            -self.config.max_forward_steer,
            self.config.max_forward_steer,
        )
        return self._emit(base_speed + steer, base_speed - steer, mode_name)

    def _approach_gate_command(self, visual, mode_name, pass_mode=False):
        abs_offset = abs(visual.gate_offset)
        width_ratio = visual.gate_width_ratio
        if pass_mode:
            base_speed = int(
                clamp(
                    self.config.follow_speed * (0.62 + width_ratio * 0.90 - abs_offset * 0.25),
                    max(12, int(self.config.follow_speed * 0.56)),
                    self.config.follow_speed,
                )
            )
            steer_gain = self.config.marker_steer_gain * 10.0
        else:
            base_speed = int(
                clamp(
            ...
            steer_gain = self.config.marker_steer_gain * 13.5
        steer = clamp(
            visual.gate_offset * steer_gain,
            -self.config.max_forward_steer,
            self.config.max_forward_steer,
        )
        return self._emit(base_speed + steer, base_speed - steer, mode_name)
    ...
    def update(self, reading, visual, now):
        if visual.gate_visible:
            ...
            self.last_gate_offset = visual.gate_offset
        ...
        if self.mode in (SEARCH_FORWARD, SEARCH_NEXT):
            if visual.gate_visible:
                ...
                if abs(visual.gate_offset) >= 0.55:
                    return self._align_spin_command(visual.gate_offset, ALIGN_GATE)
                return self._approach_gate_command(visual, ALIGN_GATE, pass_mode=False)
        ...
        if self.mode == ALIGN_GATE:
            if visual.gate_visible:
                ...
                if self.align_ready_hits >= 2:
                    return self._commit_pass_gate(now, visual.gate_offset)
                if abs(visual.gate_offset) >= max(self.config.align_spin_offset_threshold, 0.38):
                    return self._align_spin_command(visual.gate_offset, ALIGN_GATE)
                return self._approach_gate_command(visual, ALIGN_GATE, pass_mode=False)
        ...
        if self.mode == PASS_GATE:
            if visual.gate_visible:
                self.pass_last_seen_at = now
                self.pass_last_offset = visual.gate_offset
                ...
                return self._approach_gate_command(visual, PASS_GATE, pass_mode=True)

            blind_drive_active = (now - self.pass_last_seen_at) <= self.config.gate_blind_drive_seconds
            if blind_drive_active:
                return self._gate_blind_drive_command(self.pass_last_offset, PASS_GATE)
            ...
            if passed_gate or timed_out:
                ...
                self._enter_search_mode(SEARCH_FORWARD, now)
                return self._search_command(SEARCH_FORWARD, now)
            return self._open_area_forward_command(visual, PASS_GATE)
...
def draw_overlay(frame, reading, visual, command, navigator, fps, run_state, config):
    ...
    lines = [
        ...
        f"{gate_text}  offset {visual.gate_offset:+.2f}  conf {visual.gate_confidence:.2f}",
    ]
...
def update_lcd(lcd_display, config, reading, visual, command):
    if config.mode == MODE_RECORD_VALIDATE:
        ...
        if reading.valid and reading.distance_cm is not None:
            line_2 = f"D{reading.distance_cm:4.1f} C{visual.gate_confidence:.2f}"
        else:
            line_2 = f"NoD C{visual.gate_confidence:.2f}"
