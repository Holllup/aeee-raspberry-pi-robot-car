...
def detect_right_purple_square(frame_bgr):
    ...
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours = [contour for contour in contours if cv2.contourArea(contour) >= PURPLE_MIN_AREA]
    if not contours:
        return None
    ...
    for contour in contours:
        ...
        candidate = {"rect": (x + roi_left, y, w, h), "area": area}
